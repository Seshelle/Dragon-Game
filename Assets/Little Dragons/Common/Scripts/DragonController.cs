...
public class DragonController : MonoBehaviour
{
    ...
    public float flyTurn = 25f;
    [Range(0.2f,1f)]
    public float flyAnimationSpeed = 1;
    public bool glideOnly = true;
    public float airResistance = 10f;
    public float speedGain = 30f;
    ...
    public float waterLine = -0.05f;
    public float swimSpeed = 10f;
    public float swimTurn = 1f;

    [Header("Underwater")]
    public float UnderSpeed = 2f;
    public float UnderTurn = 2f;
    #endregion
    ...
    void Start()
    {
        ...
        anim.SetBool("Glide Only", glideOnly);
    }
    ...
    public void AddVector(Vector3 direction, float amount)
    {
        DragoTransform.position = Vector3.Lerp(DragoTransform.position, DragoTransform.position + direction * amount, Time.deltaTime);
    }

    public void SuspendStall(bool setting)
    {
        suspendStall = setting;
    }
    ...
    void SpeedAmount()
    {
        ...
        else if(fly && glideOnly)
        {
            //create lift
            //dragoRigidBody.AddForce(DragoTransform.up * 25, ForceMode.Force);

            //if speed is too low max angle of ascent is decreased
            if (upDown > (dragoRigidBody.velocity.magnitude / stallSpeed - stallDecay - 0.5f) && !suspendStall)
            {
                upDown = dragoRigidBody.velocity.magnitude / stallSpeed - stallDecay - 0.5f;
                stallDecay += Time.deltaTime;
            }
            else if (stallDecay > 0)
            {
                stallDecay -= Time.deltaTime;
            }
            ...
            if (!suspendStall)
            {
                velocity = velocity - (upDown * Time.deltaTime * speedGain);
                if (stallDecay <= 0)
                {
            ...
            if (velocity > dragoRigidBody.velocity.magnitude * 3f)
            {
                //stallDecay = 1f;
                fly = false;
            }
            ...
            direction = (direction + Vector3.up * upDown).normalized;
            amount = velocity * 2;
        ...
        else
        {
            ...
            //if (anim.applyRootMotion == false || anim.GetCurrentAnimatorStateInfo(0).IsTag("Idle"))
            if ((vertical == 0 && horizontal == 0) || anim.applyRootMotion == false)
            {
                amount = 0;
            }
        }
        ...
        if ((fly && glideOnly))
        {
            dragoRigidBody.velocity = Vector3.Lerp(dragoRigidBody.velocity, direction * amount * axis / 5f, Time.deltaTime * 10f);
        }
        else
        {
            dragoRigidBody.AddForce(direction * amount * axis * 20, ForceMode.Force);
    ...
    void Falling()
    {
        ...
        if (Physics.Raycast(dragoCollider.bounds.center, -Vector3.up, out hitpos, 0.9f * scaleFactor, GroundLayer))
        {
    ...
    void Swimming()
    {
        ...
        //Front RayWater Cast
        if (Physics.Raycast(pivots[2].transform.position, /*-DragoTransform.up*/ -Vector3.up, out WaterHitCenter, dragoHeight * scaleFactor * 3, LayerMask.GetMask("Water")))
        {
        ...
        if (swim)
        {
            ...
            {
                dragoRigidBody.useGravity = true;
                if (waterlevel - DragoTransform.position.y < 2f && waterlevel - DragoTransform.position.y > 0 && isHit)
                {
        ...
        if (!isInWater && !isJumping(0.6f, true))
        {
            swim = false;
            underWater = false;
            dragoRigidBody.constraints = RigidbodyConstraints.FreezeRotationZ | RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationY;
    ...
    void UnderWaterMovement()
    {
        ...
        transform.Rotate(Vector3.up, UnderTurn * horizontal * Time.fixedDeltaTime, Space.World);

        //transform.Rotate(Vector3.up, 100 * horizontal * Time.fixedDeltaTime, Space.World);

        if ((Vector3.Angle(transform.forward, Vector3.up) > 30 && jump) || (Vector3.Angle(transform.forward, Vector3.up) < 170 && down)
            || (UpDownAxis && cameraMove && Vector3.Angle(transform.forward, Vector3.up) < 170  && Vector3.Angle(transform.forward, Vector3.up) > 30)) //Limit Up Down Axis
        {
            transform.Rotate(transform.right, 200 * -upDown * Time.fixedDeltaTime, Space.World);
        }

        //fix z rotation
        if (Mathf.Abs(Vector3.Angle(transform.up, Vector3.up)) > 5 + Mathf.Abs(Vector3.Angle(transform.forward, Vector3.up) - 90))
        {
            if (Vector3.Angle(transform.right, Vector3.up) < Vector3.Angle(transform.right, -Vector3.up))
            {
                transform.Rotate(transform.forward, -200 * Time.fixedDeltaTime, Space.World);
            }
            else
            {
                transform.Rotate(transform.forward, 200 * Time.fixedDeltaTime, Space.World);
    ...
    void Grounded()
    {
        ...
        if (Physics.Raycast(pivots[1].transform.position, -Vector3.up, out hitGrounded, dragoHeight * 1.1f * scaleFactor, GroundLayer))
        {
            //Debug.DrawRay(pivots[1].transform.position, -Vector3.up * dragoHeight * scaleFactor, Color.blue);
            if (isJumping(0.3f, true))
            {
                grounded = false;
                //grounded = true;
            }
    ...
    void Update()
    {
        ...
        if (cameraMove)
        {
            ...
            {
                vertical = Mathf.Max(forwardAmount, vertical);
            }
        ...
        if (isJumping(0.3f, true)) grounded = false;

        //jump in air to toggle fly
        if (glideOnly)
        {
            if (!jump) jumpHeldLastFrame = false;

            if (fly && jump && !jumpHeldLastFrame)
            {
                //fly = false;
            }
            else if ((isJumping() || fall) && !fly && jump && !jumpHeldLastFrame)
            {
                fly = true;
                stallDecay = 0f;
                dragoRigidBody.useGravity = false;
                if (isJumping())
                {
                    dragoRigidBody.velocity = (DragoTransform.forward + Vector3.up * upDown).normalized * startSpeed;
                    velocity = startSpeed;
                }
                else
                {
                    velocity = dragoRigidBody.velocity.magnitude * 2.5f;
                }
                //dragoRigidBody.velocity = DragoTransform.forward * 10f;
                //dragoRigidBody.drag = 20;
            }

            if (jump) jumpHeldLastFrame = true;
        }

        if (!fly) dragoRigidBody.useGravity = true;
